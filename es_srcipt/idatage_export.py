...
class Espusher(object):
    def __init__(self):
        ...
        with open(os.path.abspath('config.json'), 'r') as f:
            config = json.load(f)
        #self.client = MongoClient('mongodb://127.0.0.1:27017/admin?maxPoolSize=8')#config['mongo_config']['MongoHost']
    ...
    #导出twitter用户的所有文章
    def run_twitter_pusher(self,project={}):
            while True:
                # print('开始跑程序')
                try:
                    ...
                    for item in es_data:
                        try:
                            # print(item)
                            item['created_at'] = datetime.strptime(item['created_at'], '%a %b %d %H:%M:%S %z %Y').strftime('%Y-%m-%dT%H:%M:%S.000Z')
                            ...
                            if bool(result[0].get("success", False)):
                                # item['es_pushed'] = True
                                update_doc = self.dbs.Tuser_post.find_one_and_update({'_id': item['_id']},{'$set':{'es_pushed':True}},return_document=ReturnDocument.AFTER)
